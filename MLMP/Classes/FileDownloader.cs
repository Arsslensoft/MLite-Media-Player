...
namespace FileDownloaderApp
{
    ...
    internal class FileDownloader : Object, IDisposable
    {
        #region Nested Types
        ...
        public struct FileInfo
        {
            ...
            {
                Path = path;
                Name = Path.Split("/"[0])[Path.Split("/"[0]).Length - 1];
            }
        }

        #endregion

        #region Nested type: InvokeType
        ...
        private Int64 m_currentFileProgress;
        private Int64 m_currentFileSize;
        private Int32 m_currentSpeed;
        private Boolean m_deleteCompletedFiles;
        private Boolean m_disposed = false;
        private Int32 m_fileNr;
        ...
        private Int32 m_stopWatchCycles;
        private Boolean m_supportsProgress;
        private Int64 m_totalProgress;
        ...
        private void bgwDownloader_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            while (fileNr < Files.Count && !bgwDownloader.CancellationPending)
            {
                ...
                if (bgwDownloader.CancellationPending)
                {
                    ...
                    cleanUpFiles(DeleteCompletedFilesAfterCancel ? 0 : m_fileNr,
                                 DeleteCompletedFilesAfterCancel ? m_fileNr + 1 : 1);
                }
                else
                {
                    fileNr += 1;
        ...
        private void calculateFilesSize()
        {
            ...
            for (Int32 fileNr = 0; fileNr < Files.Count; fileNr++)
            {
                ...
                try
                {
                    var webReq = (HttpWebRequest) WebRequest.Create(Files[fileNr].Path);
        ...
        private void downloadFile(Int32 fileNr)
        {
            ...
            var readBytes = new Byte[PackageSize];
            Int32 currentPackageSize;
            var speedTimer = new Stopwatch();
            Int32 readings = 0;
            Exception exc = null;

            var writer = new FileStream(LocalDirectory + "\\" + file.Name, FileMode.Create);
            ...
            try
            {
                webReq = (HttpWebRequest) WebRequest.Create(Files[fileNr].Path);
            ...
            else
            {
                ...
                while (m_currentFileProgress < size && !bgwDownloader.CancellationPending)
                {
                    ...
                    {
                        m_currentSpeed =
                            (Int32) (PackageSize*StopWatchCyclesAmount*1000/(speedTimer.ElapsedMilliseconds + 1));
                        speedTimer.Reset();
                        readings = 0;
                    }
                }
        ...
        private void bgwDownloader_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            switch ((InvokeType) e.ProgressPercentage)
            {
                case InvokeType.EventRaiser:
                    switch ((Event) e.UserState)
                    {
                        case Event.CalculationFileSizesStarted:
                            if (CalculationFileSizesStarted != null)
                            {
                                CalculationFileSizesStarted(this, new EventArgs());
                            }
                            break;
                        case Event.FileSizesCalculationComplete:
                            if (FileSizesCalculationComplete != null)
                            {
                                FileSizesCalculationComplete(this, new EventArgs());
                            }
                            break;
                        case Event.DeletingFilesAfterCancel:
                            if (DeletingFilesAfterCancel != null)
                            {
                                DeletingFilesAfterCancel(this, new EventArgs());
                            }
                            break;

                        case Event.FileDownloadAttempting:
                            if (FileDownloadAttempting != null)
                            {
                                FileDownloadAttempting(this, new EventArgs());
                            }
                            break;
                        case Event.FileDownloadStarted:
                            if (FileDownloadStarted != null)
                            {
                                FileDownloadStarted(this, new EventArgs());
                            }
                            break;
                        case Event.FileDownloadStopped:
                            if (FileDownloadStopped != null)
                            {
                                FileDownloadStopped(this, new EventArgs());
                            }
                            break;
                        case Event.FileDownloadSucceeded:
                            if (FileDownloadSucceeded != null)
                            {
                                FileDownloadSucceeded(this, new EventArgs());
                            }
                            break;
                ...
                case InvokeType.FileDownloadFailedRaiser:
                    ...
                    {
                        FileDownloadFailed(this, (Exception) e.UserState);
                    }
                    break;
                case InvokeType.CalculatingFileNrRaiser:
                    if (CalculatingFileSize != null)
                    {
                        CalculatingFileSize(this, (Int32) e.UserState);
                    }
                    break;
            }
        }
            ...
            set
            {
                if (IsBusy)
                {
                    throw new InvalidOperationException(
                        "You can not change the SupportsProgress property during the download");
                }
                else
                {
            ...
            set
            {
                if (IsBusy)
                {
                    if (IsPaused != value)
                    {
                        ...
                        {
                            IsPausedChanged(this, new EventArgs());
                        }
                        if (StateChanged != null)
                        {
                            StateChanged(this, new EventArgs());
                ...
                {
                    throw new InvalidOperationException(
                        "You can not change the IsPaused property when the FileDownloader is not busy");
                }
            }
        }
        ...
        public Int64 TotalSize
        {
            get
            {
                ...
                {
                    throw new InvalidOperationException(
                        "This FileDownloader that it doesn't support progress. Modify SupportsProgress to state that it does support progress to get the total size.");
                }
            }
        }
        ...
        public Double TotalPercentage(Int32 decimals)
        {
            ...
            {
                throw new InvalidOperationException(
                    "This FileDownloader that it doesn't support progress. Modify SupportsProgress to state that it does support progress.");
            }
        }
